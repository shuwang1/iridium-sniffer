...
"\n"
"/* Elliptical beam footprint: Iridium spot beams are circular at nadir\n"
" * but stretch into ellipses at off-nadir angles. Given the satellite\n"
" * sub-satellite point (nadir) and the beam center on the ground, compute\n"
" * semi-major (radial stretch) and semi-minor axes + bearing angle. */\n"
"var Re=6371;\n"
"function rad(d){return d*Math.PI/180}\n"
"function deg(r){return r*180/Math.PI}\n"
"\n"
"/* Great-circle distance (radians) and bearing between two lat/lon points */\n"
"function gcDistBear(lat1,lon1,lat2,lon2){\n"
"  var f1=rad(lat1),f2=rad(lat2),dl=rad(lon2-lon1);\n"
"  var sd=Math.acos(Math.min(1,Math.sin(f1)*Math.sin(f2)\n"
"    +Math.cos(f1)*Math.cos(f2)*Math.cos(dl)));\n"
"  var y=Math.sin(dl)*Math.cos(f2);\n"
"  var x=Math.cos(f1)*Math.sin(f2)-Math.sin(f1)*Math.cos(f2)*Math.cos(dl);\n"
"  var b=Math.atan2(y,x);\n"
"  return {dist:sd,bear:b};\n"
"}\n"
"\n"
"/* Build an ellipse polygon (N points) given center, semi-axes in km, bearing */\n"
"function beamEllipse(lat,lon,semiMajKm,semiMinKm,bearRad,N){\n"
"  N=N||32;\n"
"  var pts=[];\n"
"  var f1=rad(lat),l1=rad(lon);\n"
"  for(var i=0;i<=N;i++){\n"
"    var a=2*Math.PI*i/N;\n"
"    var rKm=semiMajKm*semiMinKm/Math.sqrt(\n"
"      Math.pow(semiMinKm*Math.cos(a),2)+Math.pow(semiMajKm*Math.sin(a),2));\n"
"    var ar=a+bearRad;\n"
"    var d=rKm/Re;\n"
"    var f2=Math.asin(Math.sin(f1)*Math.cos(d)\n"
"      +Math.cos(f1)*Math.sin(d)*Math.cos(ar));\n"
"    var l2=l1+Math.atan2(Math.sin(ar)*Math.sin(d)*Math.cos(f1),\n"
"      Math.cos(d)-Math.sin(f1)*Math.sin(f2));\n"
"    pts.push([deg(f2),deg(l2)]);\n"
"  }\n"
"  return pts;\n"
"}\n"
"\n"
"/* Given satellite nadir lat/lon/alt and beam center lat/lon,\n"
" * return {major, minor, bear} for the beam footprint ellipse (km).\n"
" * Nadir angle alpha = atan2(rr*sin(theta), 1-rr*cos(theta)) where\n"
" * rr = Re/(Re+h). Ground incidence angle = alpha+theta. Radial\n"
" * stretch = 1/cos(incidence) from oblique Earth intersection. */\n"
"function beamShape(nadirLat,nadirLon,satAlt,beamLat,beamLon){\n"
"  var baseR=200;\n"
"  var gc=gcDistBear(nadirLat,nadirLon,beamLat,beamLon);\n"
"  var theta=gc.dist;\n"
"  if(theta<0.01)return{major:baseR,minor:baseR,bear:0};\n"
"  var h=satAlt||780;\n"
"  var sinT=Math.sin(theta),cosT=Math.cos(theta);\n"
"  var rr=Re/(Re+h);\n"
"  var alpha=Math.atan2(rr*sinT,1-rr*cosT);\n"
"  var stretch=1/Math.cos(alpha+theta);\n"
"  if(stretch>6)stretch=6;\n"
"  if(stretch<1)stretch=1;\n"
"  return{major:baseR*stretch,minor:baseR,bear:gc.bear};\n"
"}\n"
"\n"
...
"  var cut=now-TW;\n"
"\n"
"  /* Build satellite nadir lookup from orbital positions (most recent per sat) */\n"
"  var nadir={};\n"
"  if(d.ra)d.ra.forEach(function(p){\n"
"    if(!nadir[p.sat]||p.t>nadir[p.sat].t)nadir[p.sat]=p;\n"
"  });\n"
"\n"
...
"      var nd=nadir[parseInt(sid)];\n"
"      pts.forEach(function(pt){\n"
"        var age=(now-pt.t)/TW;\n"
"        var sh=nd?beamShape(nd.lat,nd.lon,nd.alt,pt.lat,pt.lon)\n"
"          :{major:200,minor:200,bear:0};\n"
"        L.polygon(beamEllipse(pt.lat,pt.lon,sh.major,sh.minor,sh.bear),{\n"
